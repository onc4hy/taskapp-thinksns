 ...
 */
     ...
     * @return object|array
     */
    public function getTaskList($map = null,$field=null,$order = null,$limit = null) {
    //处理where条件
	    $map = $this->merge( $map );
        //连贯查询.获得数据集
    ...
    public function getAllNoticesData($where,$order){
            ...
            //根据以上处理条件获取数据集
            $result             = $this->getNoticeList($where,null,$order);
            $result['category'] = $this->getCategory();
            return $result;
    }
	
    public function getNoticeList($where = null,$field=null,$order = null,$limit = null) {
		$dao = D('taskNotices');
		//处理where条件
	    //$map = $this->merge( $map );
        //连贯查询.获得数据集
    ...
    public function getAllNotesData($where,$order){
            ...
            //$temp_order_map = $this->getOrderMap($order);
            //根据以上处理条件获取数据集
            $result             = $this->getNoteList($where,null,$order);
            $result['category'] = $this->getCategory();
			
            return $result;
    }
	
    public function getNoteList($where = null,$field=null,$order = null,$limit = null) {
		$dao = D('taskNotes');
		//处理where条件 
     ...
     * @return void
     */
    public function getMentionTask( $uid = null ) {
  ...
  public function getIsHot() {  //获取推荐博客...重复//TS_2.0
    ...
    public function getFollowsTask($mid){
    	$fl=D("Follow");
    ...
    private function getOrderMap($order){
           switch( $order ) {
                ...
                case 'hot':    //推荐阅读
                    $map['isHot'] = 1;
                    $order        = 'rTime DESC,mTime DESC';
                    break;

                case 'new':    //最新排行
                    $order = 'cTime DESC,mTime DESC';
                    break;

                case 'popular':    //人气排行
                    $order        = 'hot DESC,mTime DESC';
                    $map['cTime'] = self::_orderDate( $this->config->allorder );//取得时间
                    break;

                case 'read':   //阅读排行
                    $order = 'readCount DESC,mTime DESC';
                    $map['cTime'] = self::_orderDate( $this->config->allorder );//取得时间
                    break;

                case 'comment':   //评论排行

                    $order = 'commentCount DESC,mTime DESC';
                    $map['cTime'] = self::_orderDate( $this->config->allorder );//取得时间
                    break;
     ...
     * @return void
     */
    public function doDeleteTask( $map = null,$uid=null ) {
    //获得配置信息
...
//      $result = $this->api->space_changeCount( 'task',$count );
    ...
    public function doSaveTask( $map,$taskid ) {
	...
	public function doAddDetail($map,$import) {
		$dao = D("taskDetail");
		$addId = $dao->add($map);
	}

	public function doRemoveDetail($map,$import) {
		$dao = D("taskDetail");
		$effectRows = $dao->where($map)->delete();
	}
	
	public function doUpdateDetail($map,$import) {
		$dao = D("taskDetail");
		$effectRows = $dao->save($map);
	}
	
	
	public function doRemoveChild($map) {
		$dao = D("tasks");
		$effectRows = $dao->save($map);
	}
